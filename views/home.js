// ==========================================================================
// BHARAT DIGITAL ASSETS - PORTFOLIO VIEW & NATIVE COMPILER INFRASTRUCTURE v6.5
// ==========================================================================
        ...
        return `
            <div id="view-home" class="view-panel space-y-5 bg-slate-50/40" style="display: none; width: 100%;">
                ...
                <div class="flex justify-between items-center bg-slate-950 text-white p-5 -mx-5 -mt-5 rounded-b-[2.25rem] shadow-xl">
                    ...
                    <button onclick="openChatbot()" class="w-10 h-10 bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-center text-slate-300 active:scale-95 transition"><i class="fa-solid fa-headset text-sm"></i></button>
                </div>
                ...
                <div class="bda-gradient-card text-white p-6 space-y-4 relative overflow-hidden">
                    ...
                    <div class="pt-2 border-t border-white/10 flex justify-between items-center z-10 relative">
                        ...
                        <button id="btn-pay-modal" onclick="openPaymentGateway()" class="px-4 py-2.5 bg-white text-slate-950 font-black rounded-xl text-xs active:scale-95 transition">Deposit Unit</button>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="bg-white p-4 rounded-2xl border shadow-sm flex items-center justify-between">
                        ...
                        <button onclick="downloadExecutedAgreementPDF()" class="w-9 h-9 bg-slate-50 border rounded-xl text-slate-500 flex items-center justify-center active:bg-slate-100 transition"><i class="fa-solid fa-download text-xs"></i></button>
                    </div>
                    <div class="bg-white p-4 rounded-2xl border shadow-sm flex items-center justify-between">
                        <div class="flex items-center gap-3">
...
window.updateDashboardDisplayValues = function() {
    ...
    if (u.isPaid) {
        ...
        dBtn.disabled = false; 
        dBtn.className = "w-9 h-9 bg-amber-50 border border-amber-200 rounded-xl text-amber-600 flex items-center justify-center active:bg-amber-100 transition";
...
// ROYAL GOLDEN HIGH-TECH AGREEMENT PDF COMPILER
window.downloadExecutedAgreementPDF = function() {
    ...
    try {
        ...
        let clauses = `This official statutory charter records that the contracting entity member ${u.name.toUpperCase()} (Registered Mobile Parameter Node: +91-${u.phone}) has voluntarily parsed demographical metadata elements on date stamp ${u.registrationDate}.\n\n` +
